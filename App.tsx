 ...
 * Usamos um serviço de JSON compartilhado para que todos os membros da IPAC vejam os mesmos dados.
 */
const BIN_ID = '67c06283e41b4d34e4a0593b'; // ID de bin atualizado
...
const App: React.FC = () => {
  ...
  // FUNÇÃO: Carregar da Nuvem com Fallback Local
  const loadFromCloud = useCallback(async (showLoading = false) => {
    ...
    try {
      ...
      console.error('Erro na nuvem, usando local:', e);
      // Fallback para localStorage se a nuvem falhar
      const local = localStorage.getItem(STORAGE_KEY);
      if (local && !isUpdatingRef.current) {
  ...
  // FUNÇÃO: Salvar na Nuvem e Local
  const saveToCloud = async (updatedData: AppData) => {
    setSyncStatus('syncing');
    isUpdatingRef.current = true;
    
    // Salva logo no local para resposta imediata
  ...
  useEffect(() => {
    loadFromCloud(true);
    // Polling a cada 15 segundos para manter todos sincronizados
  ...
  const askDeleteMinister = (id: string, name: string) => {
    setConfirmModal({
      ...
      message: `Remover ${name}? Isso apagará todas as músicas dele para todos.`,
      onConfirm: () => {
  ...
  const askDeleteScaleImage = (id: string) => {
    setConfirmModal({
      ...
      message: 'Deseja remover esta foto da escala global?',
      onConfirm: () => {
  ...
  // FIX: Adicionando a função faltante para salvar a informação do ensaio
  ...
  return (
    <div className="min-h-screen pb-20 bg-slate-50 text-slate-900 font-sans">
      {/* Navbar Global */}
      <header className="bg-white border-b border-slate-200 sticky top-0 z-40 px-4 py-3 md:px-8 shadow-sm">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-3">
            <div className="bg-indigo-600 p-2.5 rounded-2xl text-white shadow-lg shadow-indigo-200">
              <Music size={22} />
            </div>
            <h1 className="text-xl md:text-2xl font-brand font-bold text-slate-900 tracking-tight">
          ...
          <div className="flex items-center gap-2">
            ...
            {syncStatus === 'error' && (
              <button 
                ...
                className="flex items-center gap-2 text-[10px] font-bold text-red-600 bg-red-50 px-3 py-1.5 rounded-full border border-red-100 hover:bg-red-100 transition-colors"
              >
                <WifiOff size={12} />
                <span>ERRO - RECONECTAR?</span>
              </button>
            )}
          </div>
        </div>
      ...
      <main className="max-w-6xl mx-auto p-4 md:p-8">
        {/* Abas Superiores */}
        <div className="flex bg-white p-1 rounded-2xl shadow-sm border border-slate-200 mb-8 max-w-xs mx-auto">
          <button
        ...
        {activeTab === 'songs' ? (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
            <div className="flex justify-between items-center px-1">
              ...
              >
                <Plus size={20} />
                <span className="hidden sm:inline font-bold text-sm">Novo Ministro</span>
              </button>
            </div>
            ...
            <div className="mt-16 bg-white border border-slate-200 rounded-[2.5rem] p-8 shadow-sm">
              ...
              {isEditingRehearsal ? (
                <textarea
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-4 text-sm font-medium focus:ring-2 focus:ring-indigo-500 focus:outline-none min-h-[140px] transition-all"
                  autoFocus
                />
              ) : (
                <div className="space-y-6">
                  <p className="text-sm text-slate-600 leading-relaxed font-medium whitespace-pre-line bg-slate-50 p-6 rounded-2xl border border-slate-100">
                    {data.rehearsalInfo}
                  </p>
                  <div className="flex items-center gap-4 bg-red-50 p-5 rounded-2xl border border-red-100">
                    ...
                    <p className="text-xs font-bold text-red-700 leading-snug">
                      ATENÇÃO: Atrasos acima de 10 min prejudicam o ensaio de todos. Por favor, avise se houver imprevistos.
                    </p>
                  </div>
                </div>
              )}
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
            ...
            {data.scaleImages.length === 0 ? (
              <div className="text-center py-28 bg-white rounded-[2.5rem] border-2 border-dashed border-slate-200">
                <ImageIcon size={48} className="mx-auto text-slate-300 mb-4" />
                <p className="text-slate-400 font-bold text-sm">Nenhuma foto da escala postada ainda.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                {data.scaleImages.map((img) => (
                  <div key={img.id} className="bg-white p-4 rounded-[2rem] shadow-lg border border-slate-100 group relative">
                    <img src={img.url} alt="Escala" className="w-full rounded-[1.5rem] object-contain bg-slate-50 max-h-[600px] shadow-inner" />
                    <div className="mt-4 flex justify-between items-center px-2">
                      <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Publicado em: {img.date}</span>
                      <button onClick={() => askDeleteScaleImage(img.id)} className="bg-red-50 text-red-500 hover:bg-red-500 hover:text-white transition-all p-2 rounded-xl">
                        <Trash2 size={18} />
                      </button>
                    </div>
                  </div>
      ...
      {showAddMinisterModal && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-md z-[60] flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-sm p-8 animate-in zoom-in-95 duration-200">
            ...
            <h3 className="text-2xl font-black text-slate-800 text-center mb-2 tracking-tight">Adicionar Ministro</h3>
            <p className="text-slate-400 text-center text-sm mb-8">Digite o nome para criar o bloco de músicas.</p>
            <input 
              type="text" 
              value={newMinisterName}
              onChange={(e) => setNewMinisterName(e.target.value)}
              placeholder="Ex: Alisson Santos"
              className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-4 text-slate-800 font-bold focus:ring-4 focus:ring-indigo-500/10 focus:outline-none mb-8 transition-all"
              autoFocus
            />
            <div className="grid grid-cols-2 gap-4">
              <button onClick={() => setShowAddMinisterModal(false)} className="bg-slate-100 text-slate-500 font-black py-4 rounded-2xl hover:bg-slate-200">Cancelar</button>
              <button onClick={handleAddMinister} className="bg-indigo-600 text-white font-black py-4 rounded-2xl hover:bg-indigo-700 shadow-lg shadow-indigo-600/20">Confirmar</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showAddSongModal && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-md z-[60] flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-sm p-8 animate-in zoom-in-95 duration-200">
            ...
            <p className="text-slate-400 text-center text-xs mb-8 uppercase tracking-widest font-bold">Campos Obrigatórios *</p>
            
            <div className="space-y-5 mb-10">
              <div>
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2 mb-1.5 block">Nome da Música *</label>
                <input 
                  type="text" 
                  value={newSongTitle}
                  onChange={(e) => setNewSongTitle(e.target.value)}
                  placeholder="Nome do louvor..."
                  className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-4 text-slate-800 font-bold focus:ring-4 focus:ring-indigo-500/10 focus:outline-none transition-all"
                  autoFocus
                />
              </div>
              
              <div>
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2 mb-1.5 block">Nome do Cantor/Banda *</label>
                <input 
                  type="text" 
                  value={newSongArtist}
                  onChange={(e) => setNewSongArtist(e.target.value)}
                  placeholder="Ex: Anderson Freire..."
                  className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-4 text-slate-800 font-bold focus:ring-4 focus:ring-indigo-500/10 focus:outline-none transition-all"
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <button onClick={() => setShowAddSongModal(null)} className="bg-slate-100 text-slate-500 font-black py-4 rounded-2xl hover:bg-slate-200">Sair</button>
              <button 
      ...
      {confirmModal.show && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-md z-[70] flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-sm p-8 animate-in zoom-in-95 duration-200">
            ...
            <p className="text-slate-500 text-center text-sm mb-8 px-2 leading-relaxed">{confirmModal.message}</p>
            <div className="grid grid-cols-2 gap-4">
              <button onClick={() => setConfirmModal(prev => ({ ...prev, show: false }))} className="bg-slate-100 text-slate-500 font-black py-4 rounded-2xl hover:bg-slate-200">Manter</button>
...
const MinisterCard: React.FC<{ 
  ...
  return (
    <div className="bg-white rounded-[2.5rem] shadow-md border border-slate-100 overflow-hidden flex flex-col hover:shadow-xl transition-all h-full">
      <div className="bg-slate-50/80 p-6 border-b border-slate-100 flex justify-between items-center">
        ...
        <button onClick={onDelete} className="text-slate-300 hover:text-red-500 p-2 rounded-full hover:bg-red-50 transition-all">
          <X size={22} />
        </button>
      </div>
      ...
      <div className="p-6 flex-1 space-y-4">
        {minister.songs.length === 0 ? (
          <div className="py-10 text-center text-slate-400 font-medium text-sm italic bg-slate-50/50 rounded-3xl border border-dashed border-slate-200">Nenhum louvor no bloco.</div>
        ) : (
          minister.songs.map((song) => (
            <div key={song.id} className="bg-slate-50 border border-slate-100 rounded-[1.5rem] p-5 space-y-4 hover:border-indigo-200 transition-colors group">
              <div className="flex justify-between items-start">
                <div className="flex-1 space-y-1">
                  <input 
                    ...
                    className="bg-transparent border-none font-bold text-slate-900 focus:ring-0 p-0 text-base w-full placeholder:text-slate-300"
                  ...
                  <div className="flex items-center gap-1.5 text-slate-400">
                    ...
                    <input 
                      ...
                      className="bg-transparent border-none text-xs font-semibold focus:ring-0 p-0 w-full placeholder:text-slate-300"
                      placeholder="Cantor"
                    />
                  </div>
                </div>
              ...
              <div className="flex flex-wrap items-end gap-3 pt-1">
                <div className="flex-1 min-w-[120px]">
                  <label className="text-[9px] font-black text-indigo-400 uppercase tracking-widest ml-2 mb-1.5 block">Selecione o seu Tom</label>
                  <div className="relative">
                    <select
                      value={song.key}
                      onChange={(e) => onUpdateSong(song.id, { key: e.target.value as MusicKey })}
                      className="w-full appearance-none bg-white border border-slate-200 text-slate-700 text-[10px] font-black rounded-xl px-4 py-2.5 pr-10 focus:ring-2 focus:ring-indigo-500 cursor-pointer uppercase tracking-widest"
                    >
                ...
                <div className="flex-1 min-w-[160px] relative">
                  <input 
                    ...
                    className="w-full text-[10px] font-bold bg-white border border-slate-200 rounded-xl py-2.5 pl-9 pr-3 focus:ring-2 focus:ring-red-500 placeholder:text-slate-300"
                    placeholder="LINK YOUTUBE"
                  />
                  <Youtube size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-red-500" />
                  {song.youtubeLink && (
                    <a href={song.youtubeLink.includes('http') ? song.youtubeLink : `https://www.youtube.com/results?search_query=${song.title} ${song.artist}`} target="_blank" className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-red-600 transition-colors p-1">
      ...
      <div className="p-6 pt-0">
        <button onClick={onAddSong} className="w-full py-4 border-2 border-dashed border-indigo-100 rounded-2xl text-indigo-500 font-black text-[10px] uppercase tracking-widest hover:bg-indigo-50 hover:border-indigo-300 transition-all flex items-center justify-center gap-2">
          <Plus size={18} /> ADICIONAR LOUVOR
        </button>
      </div>
    </div>
  );
